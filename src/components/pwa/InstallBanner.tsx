...
export default function InstallBanner() {
  ...
  useEffect(() => {
    ...
    if (isIOS) {
      setIsIOSDevice(true);
      // Show install banner for iOS users after a brief delay
      const timer = setTimeout(() => setShowBanner(true), 2000);
      return () => clearTimeout(timer);
    } else {
      // 4. Handle Android/Desktop Chrome/Edge prompt
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                <p className="text-xs font-semibold text-blue-600 dark:text-blue-400 tracking-wider uppercase">App Available</p>
                <p className="text-sm font-bold text-slate-800 dark:text-slate-100">Install Warren App</p>
        ...
        <div className="fixed inset-0 z-[100] flex items-end justify-center bg-slate-950/40 backdrop-blur-sm transition-opacity duration-300">
          ...
          <div className="relative w-full max-w-lg rounded-t-[32px] border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 pb-8 shadow-2xl animate-slide-up text-left">
            ...
            <div className="flex items-start justify-between mb-4">
              <div>
                <h3 className="text-lg font-bold text-slate-900 dark:text-slate-50">Install on iPhone / iPad</h3>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="text-sm">
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1.5 mt-0.5">
                    Click the share button <span className="inline-flex p-1 rounded bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300"><Share size={12} /></span> at the bottom of Safari.
                  </p>
                </div>
              </div>
